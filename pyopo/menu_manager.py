...
class Menu:
        ...
        if self.selected_item != 0: # Would be 0 if there are no selectable items
            ...
            if evt_id == pygame.K_UP:
                ...
                if self.selected_item[1] == -1:
                    self.selected_item[1] = len(self.menu_items[self.selected_item[0]][1]) - 1

            elif evt_id == pygame.K_DOWN:
                # Down Arrow Key
                self.selected_item[1] += 1
                if self.selected_item[1] == len(self.menu_items[self.selected_item[0]][1]):
    ...
    def composite(self, render_surface):
        ...
        if self.cached_surface_buffer:
            ...
            render_surface.blit(self.cached_surface_buffer, (0,0,self.cached_surface_buffer.get_width(), self.cached_surface_buffer.get_height()))
        ...
        pygame.draw.rect(render_surface, (160,160,160), (3,3, menu_w - 6, menu_h - 6), 1)
        
        pygame.draw.line(render_surface, (64,64,64), (2, menu_h - 2), (menu_w - 2, menu_h - 2), 1)
        pygame.draw.line(render_surface, (64,64,64), (3, menu_h - 3), (menu_w - 3, menu_h - 3), 1)
        
        pygame.draw.line(render_surface, (64,64,64), (menu_w - 3, 3), (menu_w - 3, menu_h - 3), 1)
        ...
        for m in range(len(self.menu_items)):
            font_surface = self.menu_font_bold.render(self.menu_items[m][0], True, (0,0,0))

            if m == self.selected_item[0]:
                # mCARD is selected.
                render_surface.fill(color=(255,255,255), rect=(menu_off, 0, font_surface.get_width() + MENU_PADDING_WIDTH * 2, menu_h))
                pygame.draw.rect(render_surface, (0,0,0), (menu_off, 0, font_surface.get_width() + MENU_PADDING_WIDTH * 2, menu_h), 2)
                pygame.draw.rect(render_surface, (160,160,160), (menu_off + 3,3, font_surface.get_width() + MENU_PADDING_WIDTH * 2 - 6, menu_h - 6), 1)
       
                mcard_h = len(self.menu_items[m][1]) * (item_height + 2 * MENU_ITEM_PADDING_HEIGHT)

                max_width = 0
                for i in range(len(self.menu_items[m][1])):
                    mcard_item_w = self.menu_font_bold.size(self.menu_items[m][1][i][0] + "Ctrl + A")[0]
                ...
                # mCARD drop shadow
                render_surface.fill(color=(128,128,128), rect=(menu_off + 2, menu_h, mcard_w + 2, menu_h - 8 + mcard_h))

                # mCARD dropdown border
                render_surface.fill(color=(255,255,255), rect=(menu_off, menu_h - 6, mcard_w, menu_h - 6 + mcard_h))

                pygame.draw.rect(render_surface, (0,0,0), (menu_off, menu_h - 6, mcard_w, menu_h - 6 + mcard_h), 2)
                pygame.draw.rect(render_surface, (160,160,160), (menu_off + 3, menu_h - 3,mcard_w - 6, menu_h - 12 + mcard_h), 1)
                
                mcard_off_h = menu_h
                for i in range(len(self.menu_items[m][1])):
                    if self.selected_item[1] == i:
                        render_surface.fill(color=(0,0,0), rect=(menu_off + 5, mcard_off_h, mcard_w - 10, item_height + 2 * MENU_ITEM_PADDING_HEIGHT ))
                    ...
                    mcard_font_surface = self.menu_font_bold.render(self.menu_items[m][1][i][0], True, mcard_item_col)
                    render_surface.blit(mcard_font_surface, (menu_off + MENU_PADDING_WIDTH, mcard_off_h))
                    ...
                    mcard_font_surface = self.menu_font.render("Ctrl+" + chr(char_val), True, mcard_item_col)
                    render_surface.blit(mcard_font_surface, (menu_off + mcard_w - 10 - mcard_font_surface.get_width(), mcard_off_h))


                    mcard_off_h += item_height + MENU_ITEM_PADDING_HEIGHT


            render_surface.blit(font_surface, (menu_off + MENU_PADDING_WIDTH, MENU_PADDING_HEIGHT))
