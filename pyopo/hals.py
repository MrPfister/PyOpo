...
class hal_tim(hal):
    ...
    def process_io(self):
        ...
        for timer in self._timers:
            if timer['end_datetime'] > datetime.datetime.now():
                ...
                current_val = (datetime.datetime.now() -  timer['start_dateime']).seconds * 10

                self._data_stack.write(0, current_val, timer['var_dsf_addr'])
                print(f"Storing Timer value: {current_val} to DSF Offset {timer['var_dsf_addrt']}")
    ...
    def add_timer(self, dsf_addr: int, expire: int):
        ...
        self._timers.append({
            ...
            "end_datetime": datetime.datetime.now()+datetime.timedelta(seconds=delta_seconds),
