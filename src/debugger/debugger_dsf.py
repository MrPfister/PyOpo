...
class DebuggerDSF:
    ...
    def store_var(self, type, dsf_offset, procedure, value, size, global_var = False, array = False):
    ...
    def composite(self):
        ...
        if self.composite_required:
            ...
            pygame.draw.rect(self.debug_surface, (0,0,0,0),
                             (5, 10, DSF_VISUALISATION_BOX_WIDTH, self.height - 15), 1)
            ...
            for var in self.variables:
                ...
                var_x = 6 + int((DSF_VISUALISATION_BOX_WIDTH -2) / bytes_per_row *  int(var - var_y * bytes_per_row))
                end_x = 6 + int((DSF_VISUALISATION_BOX_WIDTH-2) / bytes_per_row *  int(var + self.variables[var]['size'] - var_y * bytes_per_row))
                ...
                pygame.draw.line(self.debug_surface, (line_col,line_col, line_col, 0), (var_x, var_y), (end_x, var_y), 1)


            font_surface = self.default_font.render("Data Stack Frame Debugger:", True, (0,0,0,255))

            self.debug_surface.blit(
                font_surface, (5, 9 - font_surface.get_height()))
            
            pygame.draw.line(self.debug_surface, (0,0,0,0), (DSF_VISUALISATION_BOX_WIDTH + 10, 10), (self.width - 5, 10), 1)

            font_surface = self.get_text_surface("Procedure DSF variables:")
            self.debug_surface.blit(
                font_surface, (DSF_VISUALISATION_BOX_WIDTH + 10, 9 - font_surface.get_height()))
            ...
            for var in self.variables:
                ...
                font_surface = self.default_font.render(var_obj['procedure'], True, (0,0,0,255))
                self.debug_surface.blit(
                    font_surface, (DSF_VISUALISATION_BOX_WIDTH + 10, 19 - font_surface.get_height() + 8 * var_counter))
                

                font_surface = self.get_text_surface(self.type_names[var_obj['type']])
                self.debug_surface.blit(
                    font_surface, (DSF_VISUALISATION_BOX_WIDTH + 45, 19 - font_surface.get_height() + 8 * var_counter))
                
                is_global = self.is_global_var(var, var_obj['procedure'])
                font_surface = self.get_text_surface(f"Gbl - {self.global_var_name(var, var_obj['procedure'])}" if is_global else "Local")
                self.debug_surface.blit(
                    font_surface, (DSF_VISUALISATION_BOX_WIDTH + 70, 19 - font_surface.get_height() + 8 * var_counter))

                
                # The value needs to be sanitised in case a null char is present.
                font_surface = self.get_text_surface(str(var_obj['value']))
                self.debug_surface.blit(
                    font_surface, (DSF_VISUALISATION_BOX_WIDTH + 115, 19 - font_surface.get_height() + 8 * var_counter))
