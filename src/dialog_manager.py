...
ALLOWED_ALPHA_CHARS=[K_a, K_b, K_c, K_d, K_e, K_f, K_g, K_h, K_i, K_j, K_k, K_l, K_m, K_n, K_o, K_p, K_q, K_r, K_s, K_t, K_u, K_v, K_w, K_x, K_y, K_z, K_SPACE, K_COMMA, K_PERIOD]
...
class Dialog:
                        ...
                        if self.dialog_items[i]['type'] in ['dEDIT']:
                            if evt_id == 8:
                                ...
                                if len(self.dialog_items[i]['value'])>0:
                                    self.dialog_items[i]['value'] = self.dialog_items[i]['value'][:-1]
                            elif evt_id in ALLOWED_ALPHA_CHARS or evt_id in ALLOWED_NUM_CHARS:
                                # Add the character
                            ...
                            if evt_id == 8:
                                ...
                                # Add the character
                                self.dialog_items[i]['value'] += chr(evt_id)
                            elif self.dialog_items[i]['type'] == 'dFLOAT' and evt_id == K_PERIOD:
                                # dFLOAT allows for decimal
                                if not '.' in self.dialog_items[i]['value']:
                                    self.dialog_items[i]['value'] += '.'
                            elif evt_id == K_MINUS and len(self.dialog_items[i]['value']) == 0:
                            ...
                            if evt_id == 259:
                                ...
                                if self.dialog_items[i]['value'] == 0:
                                    self.dialog_items[i]['value'] = len(self.dialog_items[i]['choice_list'].split(','))

                            elif evt_id == 258:
                                # Right
                                self.dialog_items[i]['value'] += 1
                                if self.dialog_items[i]['value'] > len(self.dialog_items[i]['choice_list'].split(',')):
                                    self.dialog_items[i]['value'] = 1
                        break
    ...
    def composite(self, screen_width, screen_height):
        ...
        for item in self.dialog_items:
            item_width = 0
            if item['type'] == 'dINIT':
                item_width = self.dialog_font.size(item['title'])[0] + DIALOG_PADDING * 2
            elif item['type'] == 'dTEXT':
                item_width = self.dialog_font.size(item['p'])[0] + self.dialog_font.size(item['body'])[0] + DIALOG_PADDING * 3
            elif item['type'] in ['dEDIT', 'dLONG', 'dCHOICE', 'dFLOAT']:
                item_width = self.dialog_font.size(item['prompt'])[0] +  self.dialog_font.size('ABCDEFGHIJKL')[0] +  DIALOG_PADDING * 3
                ...
                for i in range(len(item['buttons'])):
                    ...
                    button_text = item['buttons'][i][0] + ' %' + chr(item['buttons'][i][1]) 
                    max_button_width = max(max_button_width, self.dialog_font.size(button_text)[0])
        ...
        # Render the border
        pygame.draw.rect(self.dialog_surface, (0,0,0), (0,0,self.dialog_width, self.dialog_height), 2)
        pygame.draw.rect(self.dialog_surface, (160,160,160), (4,4,self.dialog_width - 8, self.dialog_height - 8), 1)
        ...
        for i in range(len(self.dialog_items)):
            ...
            if item['type'] == 'dINIT':
                self.dialog_surface.fill(color=(192,192,192), rect=(5, height_offset, self.dialog_width - 10, item_height))

                font_surface = self.dialog_font.render(item['title'], True, (0,0,0))
                self.dialog_surface.blit(font_surface, (self.dialog_width / 2 - font_surface.get_width() / 2, height_offset + DIALOG_PADDING))
            elif item['type'] == 'dTEXT':

                if item['selectable'] and self.selected_item == item['selection_index']:
                    # Item is selected
                    print('Item is selected')
                    self.dialog_surface.fill(color=(160,160,160), rect=(DIALOG_MARGIN, height_offset, self.dialog_width - (DIALOG_MARGIN) * 2, item_height))

                d_offset = DIALOG_MARGIN + DIALOG_PADDING
                if len(item['p']) > 0:
                    font_surface = self.dialog_font.render(item['p'], True, (0,0,0))
                    self.dialog_surface.blit(font_surface, (d_offset, height_offset + DIALOG_PADDING))
                    d_offset += font_surface.get_width() + DIALOG_PADDING

                if len(item['body']) > 0:
                    if item['use_bold']:
                        font_surface = self.dialog_font_bold.render(item['body'], True, (0,0,0))
                    else:
                        font_surface = self.dialog_font.render(item['body'], True, (0,0,0))

                    if item['text_align'] == 1:
                        # Right Align Body Text
                        d_offset = self.dialog_width - DIALOG_MARGIN - font_surface.get_width() - DIALOG_PADDING
                    elif item['text_align'] == 2:
                        # Centre Align Body Text
                        d_offset = d_offset + (self.dialog_width - DIALOG_MARGIN - DIALOG_PADDING - d_offset) / 2 - font_surface.get_width() / 2

                    self.dialog_surface.blit(font_surface, (d_offset, height_offset + DIALOG_PADDING))

                    if item['draw_line']:
                        pygame.draw.line(self.dialog_surface, (0,0,0), (DIALOG_MARGIN + DIALOG_PADDING, height_offset + item_height - DIALOG_PADDING), (self.dialog_width - DIALOG_MARGIN - DIALOG_PADDING,  height_offset + item_height - DIALOG_PADDING), 1)
            elif item['type'] in ['dEDIT','dLONG', 'dFLOAT']:
                d_offset = DIALOG_MARGIN + DIALOG_PADDING

                if len(item['prompt'])>0:
                    font_surface = self.dialog_font.render(item['prompt'], True, (0,0,0))
                    self.dialog_surface.blit(font_surface, (d_offset, height_offset + DIALOG_PADDING))
                ...
                pygame.draw.rect(self.dialog_surface, render_col, (d_offset, height_offset + 1, self.dialog_width - DIALOG_MARGIN - DIALOG_PADDING - d_offset, item_height - 2), 1)
                ...
                if len(sanitised)> 0:
                    ...
                    self.dialog_surface.blit(font_surface, (d_offset + 2, height_offset + DIALOG_PADDING))
                ...
                for i in range(len(item['buttons'])):
                    ...
                    button_text_offset = (item['button_width'] - font_surface.get_width())/2

                    pygame.draw.rect(self.dialog_surface, (0,0,0), (d_offset, height_offset + 1,item['button_width'], item_height - 2), 1)

                    self.dialog_surface.blit(font_surface, (d_offset + button_text_offset, height_offset + DIALOG_PADDING))
                ...
                pygame.draw.rect(self.dialog_surface, render_col, (d_offset + 5, height_offset + 1, self.dialog_width - DIALOG_MARGIN - DIALOG_PADDING - d_offset - 10, item_height - 2), 1)
                ...
                self.dialog_surface.blit(font_surface, (d_offset + 5 + 2, height_offset + DIALOG_PADDING))
