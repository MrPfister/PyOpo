...
def qcode_print_semi(procedure, data_stack, stack):
...
def qcode_style(procedure, data_stack, stack):
...
def qcode_screen_4(procedure, data_stack, stack):
...
def qcode_print(procedure, data_stack, stack):
    op_code = procedure.get_executed_opcode()
    #_logger.debug(f"{hex(op_code)} - PRINT (i.e. newline at end of printing) - STUB")


def qcode_font(procedure, data_stack, stack):
...
def qcode_at(procedure, data_stack, stack):
...
def qcode_cls(procedure, data_stack, stack):
    #_logger.debug(f"0xA2 - CLS)")

    procedure.executable.window_manager.text_window.CLS()


def qcode_cursor(procedure, data_stack, stack):
...
def qcode_screeninfo(procedure, data_stack, stack):
